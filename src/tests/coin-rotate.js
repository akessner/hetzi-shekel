...
o.spec("RotatingCoin", function() {
  o("view ml sets style of margin-left", function() {
    var out = mq(RotatingCoin, { ml: -10, mt: 20, flip: false }).rootNode
      .children[0].attrs.style;
  ...
  o("Coin edge has 80 faces", function() {
    var out = mq(RotatingCoin).rootNode.children[0].children;
  ...
  o("flipped coin's first face is the back", function() {
    var out = mq(RotatingCoin, { flip: true }).rootNode.children[0].children;
    o(out[0].attrs.className).equals("coin_back");
  });
  o("default coin's first face is the front", function() {
    var out = mq(RotatingCoin).rootNode.children[0].children;
